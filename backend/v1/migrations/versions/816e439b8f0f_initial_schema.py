...
def upgrade() -> None:
    ...
    # Now add the deferred FK from forms -> form_versions (batch mode for SQLite compat)
    with op.batch_alter_table('forms', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_forms_published_version', 'form_versions',
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('forms', schema=None) as batch_op:
        batch_op.drop_constraint('fk_forms_published_version', type_='foreignkey')
